...
export const entities = {
  ...
  // Задачі — пункт меню живе тут же, в «Команді», а не власним спадним
  // заголовком: простір є незалежним від «Команд» контейнером зі своїм
  // довільним списком учасників (task_space_members), а не таблицею-
  // рядком форми — сам список, дошки й канбан-логіка йдуть через окремі
  // ендпоїнти (src/taskBoards.js) і власну сторінку, так само як
  // «Підключення клієнта» — тут реєстрація лише заради пункту меню й прав
  // доступу.
  task_spaces: {
    label: 'Задачі', group: 'Команда', icon: 'grid', title: 'name',
    fields: [
      { name: 'name', label: 'Назва', type: 'text', required: true, list: true },
      { name: 'created_at', label: 'Створено', type: 'datetime', readOnly: true },
    ],
