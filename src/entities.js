...
export const entities = {
  ...
  // ── Задачі ─────────────────────────────────────────────────────────────
  // Простір — незалежний від «Команд» контейнер зі своїм довільним
  // списком учасників (task_space_members), а не таблицею-рядком форми:
  // сам список, дошки й канбан-логіка живуть через окремі ендпоїнти
  // (src/taskBoards.js) і власну сторінку, так само як «Підключення
  // клієнта» — тут реєстрація лише заради пункту меню й прав доступу.
  task_spaces: {
    label: 'Простори задач', group: 'Задачі', icon: 'grid', title: 'name',
    fields: [
      { name: 'name', label: 'Назва', type: 'text', required: true, list: true },
      { name: 'created_at', label: 'Створено', type: 'datetime', readOnly: true },
    ],
