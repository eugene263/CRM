...
export function notify(kind, text, userId = null) {
  return insert('notifications', { kind, text: String(text).slice(0, 2000), user_id: userId });
}

async function sendOne(n) {
  const chat = n.user_id ? get('SELECT telegram_id FROM users WHERE id=?', n.user_id)?.telegram_id || CHAT() : CHAT();
...
export async function flushQueue(limit = 20) {
  const pending = all(`SELECT * FROM notifications WHERE status='pending' ORDER BY id LIMIT ?`, limit);
  let sent = 0;
  for (const n of pending) {
    try {
      const r = await sendOne(n);
      if (r.skipped) continue;                       // залишаємо в черзі до налаштування бота
      run(`UPDATE notifications SET status='sent', sent_at=datetime('now') WHERE id=?`, n.id);
      sent += 1;
    } catch (e) {
      run(`UPDATE notifications SET status='failed', error=? WHERE id=?`, String(e.message).slice(0, 300), n.id);
    }
  }
...
export function runChecks() {
  for (const p of all(
    ...
    if (get('SELECT value FROM settings WHERE key=?', key)) continue;
    notify('proxy_expiry', `⚠️ Проксі #${p.id} (${p.provider || '—'}, ${p.geo || '—'}) оплачено до ${p.paid_until}`);
    run('INSERT INTO settings (key,value) VALUES (?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value', key, '1');
  }
  for (const k of all(
    ...
    if (get('SELECT value FROM settings WHERE key=?', key)) continue;
    notify('plan', `📉 ${k.name}: план ${k.target} відео, факт ${k.fact}`, k.user_id);
    run('INSERT INTO settings (key,value) VALUES (?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value', key, '1');
  }
}
